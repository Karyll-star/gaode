...
def score_candidate(
    ...
    score_pilot = 1 if not c_core else min(5, 2 + len(c_core))
    eval_candidates = [h for h in core_hits if h["group"] in ("gate", "express_point", "parking", "trash_point")]
    ...
    if len(gate_like) >= 3:
        score_boundary = 5
    elif len(gate_like) == 2:
        score_boundary = 4
    elif len(gate_like) == 1:
        score_boundary = 3
    else:
...
def main():
    ...
    for row in rows:
        ...
        # 报表示例：取距离最近的 3 个 POI
        row["_sample_hits"] = [
            f"{h.get('name','')}@{h.get('distance','')}m"
            for h in sorted(hits, key=lambda x: x.get("distance") or 1e9)[:3]
            if h.get("name")
        ]
